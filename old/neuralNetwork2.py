...
class NeuralNetwork:
    ...
    def gradient_descent(self, x, y, iterations):
        for i in range(iterations):
            for input in x:
                ...
                g_wjk = Xj.T @ ((y[0][0] - yhat) * self.sigmoid_derivative(Xj, self.wjk))
                print('-'*10,'\n',g_wjk)
                exit()
                # gradients for input to hidden weights
                g_wij = Xi.T @ ((y - yhat) * self.sigmoid_derivative(Xj, self.wjk) @ self.wjk.T * self.sigmoid_derivative(Xi, self.wij))
                # update weights
                self.wij += g_wij
                self.wjk += g_wjk
    ...
    def gradient_descentORIG(self, x, y, iterations):
        for i in range(iterations):
            Xi = x
            Xj = self.sigmoid(Xi, self.wij)
            yhat = self.sigmoid(Xj, self.wjk)
